...
| `patient.age` | Int |
| `patient.gender` | String — `MALE`, `FEMALE` or `OTHER`, mapped by the bridge to DICOM Patient Sex `M`, `F`, `O`. A new survey starts on `MALE`, matching the console's own NEW STUDY screen, so the field is normally present. |
